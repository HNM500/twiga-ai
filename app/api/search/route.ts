...
export async function POST(req: Request) {
  ...
  const stream = createUIMessageStream<ChatMessage>({
      ...
      const result = streamText({
        ...
        system:
          ...
          (shouldUseXaiMultiAgent
            ...
            : ''),
        toolChoice: group === 'web' ? 'required' : 'auto',
        tools: streamTools,
        ...
        prepareStep: async ({ steps }) => {
          ...
          return {
            toolChoice: group === 'web' ? ('required' as const) : ('auto' as const),
        ...
        onFinish: async (event) => {
          ...
          logOperationalEvent('ai_generation_completed', {
            ...
            authenticated: Boolean(lightweightUser),
        ...
        onError(event) {
          ...
          logOperationalError('ai_generation_failed', event.error, {
            requestId: streamId,
            mode: group,
            model,
            durationMs: Math.round(processingTime * 1000),
    ...
    onError(error) {
      // Production builds remove console.log calls, but operational failures must
      // remain visible in Railway logs without exposing request payloads.
      logOperationalError('search_stream_failed', error, {
        requestId: streamId,
        mode: group,
        model,
