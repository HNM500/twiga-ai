...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  if (!chat || chat.visibility !== 'public') {
    return { title: 'Twiga AI Chat' };
  }

  return {
    title: chat.title,
    description: 'A shared chat on twiga.ai',
    openGraph: {
      title: chat.title,
      url: `https://twiga.ai/share/${id}`,
      description: 'A shared chat on twiga.ai',
      siteName: 'twiga.ai',
      images: [
        {
          url: `https://twiga.ai/api/og/chat/${id}`,
          width: 1200,
          height: 630,
        },
      ],
    ...
    alternates: {
      canonical: `https://twiga.ai/share/${id}`,
    },
  } as Metadata;
}
...
export default async function Page(props: { params: Promise<{ id: string }> }) {
  ...
  const shareUrl = `https://twiga.ai/share/${id}`;
  const uiMessages = convertToUIMessages(messages);

  const sharedBy = chat.userName || chat.userEmail || 'Twiga AI user';
